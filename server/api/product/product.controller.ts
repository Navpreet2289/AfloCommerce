...
export default class ProductCtrl extends BaseCtrl {
  ...
  create = (req, res) => {
    ...
    if (config.product.moderate) req.body.approved = false; else req.body.approved = true; // If the review required moderation (server/settings/environment/shared.js)
    // let now = Date.now();
